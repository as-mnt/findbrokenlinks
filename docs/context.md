...
- **Soft-404 raw target** — добавлен 4-й target `raw` (нетронутый HTML body) для паттернов,
  чьи сигнатуры живут в разметке: CSS-классы, server-signature, конкретные комбинации тегов.
  `nginx_default_404` и `tilda_404` переключены на `raw` — раньше они искали HTML в тексте
...
- **Drop unused runtime deps `jinja2` и `rich`** — оба фигурировали в плане (Jinja2 для
