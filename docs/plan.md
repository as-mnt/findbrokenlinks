...
> ⚠️ **Исторический снимок.** Это план, одобренный пользователем **до старта кодинга**
> (см. `docs/task.md` для исходной задачи). Сохраняется как точка отсчёта — что хотели,
> какие паттерны выбрали, какие зависимости планировали.
>
> **Не отражает текущее состояние кода.** Часть плана выполнена не так, как написано:
> `jinja2` и `rich` не использованы (HTML/Markdown рисуются вручную, прогресс — через
> stdlib `logging`); добавлены вещи, которых не было в плане (`ANTIBOT_BLOCKED` check,
> репортер `grouped-json`, plugin auto-discovery, `--max-pages`, content-type guard,
> classifier для SSL/DNS/timeout); порог `REDIRECT_CHAIN` зафиксирован как `>=` а не
> `>`; добавлены не входившие в план реалии (Accept-headers по умолчанию, login-walls,
> SSL chain detection).
>
> **Актуальное состояние:** `docs/context.md` — там список фичей, файлов, paттернов,
> CLI-флагов, тестов. История изменений после v0.1 с SHA коммитов — в конце файла.
